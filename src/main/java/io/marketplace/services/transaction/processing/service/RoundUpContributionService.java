...
public class RoundUpContributionService {
    ...
    private static final String BALANCE_BELOW_ZERO = "Insufficient balance";
    private static final String ACCOUNT_SUB_TYPE = "SavingAccount-i";
    private static final String ACCOUNT_TYPE = "DEPOSIT";
    ...
    public void processTransaction(OBTransaction6 transaction) {
        ...
        if (!optConfig.isPresent()) {
            log.info(
                    "Transaction not eligible to round up contribution {}",
        ...
        OBActiveOrHistoricCurrencyAndAmount9 amount =
                Optional.ofNullable(transaction)
                        .map(OBTransaction6::getAmount)
        ...
        if (roundUpAmount.compareTo(BigDecimal.ZERO) > 0) {
            ...
            RoundUpNotificationDetails roundUpNotificationDetails =
                    RoundUpNotificationDetails.builder()
                            ...
                            .build();
            try {
                WalletFundTransferRequest walletFundTransferRequest =
                        toWalletFundTransferRequest(
                                configurationEntity, roundUpAmount, amount.getCurrency());
                CustomField customField =
                        CustomField.builder()
                                .key(ROUNDUP_SOURCE_TRANSACTION_ID)
                                .value(transaction.getTransactionId())
                                .build();
                walletFundTransferRequest.setCustomFields(Collections.singletonList(customField));
                log.info(
                        "Round up WalletFundTransferRequest: {}",
                        gson.toJson(walletFundTransferRequest));

                WalletFundTransferResponse walletFundTransferResponse =
                        walletClient.walletFundTransfer(
                                walletFundTransferRequest,
                                transaction.getUserId(),
                                UseCase.ACTIVITY_RECEIVE_TRANSACTION_DATA,
                                EventCode.EVENT_RECEIVE_TRANSACTION_DATA);
                log.info(
                        "Round up WalletFundTransferResponse: {}",
                        gson.toJson(walletFundTransferResponse));
                sendPaymentNotification(roundUpNotificationDetails);
            } catch (HttpStatusCodeException httpStatusCodeException) {
                log.error(
                        ErrorCodes.ERROR_CALL_WALLET_SERVICE.getMessage(), httpStatusCodeException);
    ...
    private Optional<ConfigurationEntity> isTransactionEligible(OBTransaction6 transaction) {
        ...
        if (OBEntryStatus1Code.COMPLETED != transaction.getStatus()
            ...
            log.info(
                    "Not Eligible for round up due to Status: {} and CreditDebitIndicator: {} ",
                    transaction.getStatus().getValue(),
                    transaction.getCreditDebitIndicator().getValue());
            return optConfig;
        }
        ...
        String debitorAccountNumber =
                Optional.ofNullable(transaction)
                        .map(OBTransaction6::getDebtorAccount)
                        .map(OBCashAccount61::getIdentification)
                        .orElseThrow(
                                () ->
                                        new BadRequestException(
                                                ErrorCodes.MISSING_DEBITOR_ACCOUNT.getCode(),
                                                ErrorCodes.MISSING_DEBITOR_ACCOUNT.getMessage(),
                                                gson.toJson(transaction)));
                ...
                Stream.of(
                                (String)
                                        transaction
                                                ...
                                                .getOrDefault(MERCHANT_CATERGORY_CODE, EMPTY_VALUE),
                                Optional.ofNullable(transaction)
                                        .map(OBTransaction6::getMerchantDetails)
                                        .map(OBMerchantDetails1::getMerchantCategoryCode)
                                        .orElse(EMPTY_VALUE))
    ...
    private String getWalletIdByAccountNumber(String accountNumber) {
        ...
        List<Wallet> walletList =
                walletClient.getUserWallet(
                        requestSearchWalletDto,
                        UseCase.ACTIVITY_RECEIVE_TRANSACTION_DATA,
                        EventCode.EVENT_RECEIVE_TRANSACTION_DATA);
        return Optional.ofNullable(walletList).stream()
    ...
    private WalletFundTransferRequest toWalletFundTransferRequest(
            ConfigurationEntity configurationEntity, BigDecimal roundUpAmount, String currency) {
        String debitorWalletId = configurationEntity.getWallet();
        Optional<ConfigurationParamEntity> configurationParamEntity =
                configurationEntity.getConfigurationParamList().stream()
                        .filter(param -> contributionWalletId.equals(param.getParamName()))
                        .findFirst();
        if (!configurationParamEntity.isPresent()) {
            throw new BadRequestException(
                    ErrorCodes.ERR_SAVINGS_POT_NOT_FOUND_ERROR.getCode(),
                ...
                .transaction(
                        Transaction.builder()
                                ...
                                .creditor(
                                        Account.builder()
                                                .wallet(
                                                        WalletFundTransferRequest.Wallet.builder()
                                                                .walletId(
