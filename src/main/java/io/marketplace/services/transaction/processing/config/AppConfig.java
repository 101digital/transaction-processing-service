...
@ComponentScan({"io.marketplace.commons.gson",
        ...
        "io.marketplace.services.notification.client"
})
@Import({
  ...
  io.marketplace.services.membership.service.MembershipServiceClient.class,
  io.marketplace.commons.config.EventMessageAutoConfiguration.class,
  io.marketplace.services.pxchange.client.producer.ProcessDataProducer.class,
  io.marketplace.services.pxchange.client.service.PXChangeServiceClient.class,
  io.marketplace.services.notification.client.producer.NotificationProcessDataProducer.class,
  io.marketplace.services.notification.client.service.NotificationServiceClient.class
})
public class AppConfig {
    ...
    @Value("${http.log.header:false}")
    private Boolean httpLogHeader;

    @Value("${http.log.client.info:true}")
    private Boolean httpLogClientInfo;

    @Value("${http.log.query:true}")
    private Boolean httpLogQueryString;

    @Value("${http.log.payload:true}")
    private Boolean httpLogPayload;
    ...
    public RestTemplate restTemplate(
            @Autowired @Qualifier("gson-rest-template-customizer")
    ...
    public RestTemplate internalRestTemplate(
            @Autowired @Qualifier("gson-rest-template-customizer")
                    RestTemplateCustomizer restTemplateCustomizer,
            @Qualifier("internal-jwt-factory") JWTFactory jwtFactory) {
        RestTemplate restTemplate = new RestTemplateBuilder(restTemplateCustomizer).build();
        restTemplate
                .getInterceptors()
                .add(new InternalJwtInterceptor(jwtHeaderName, jwtFactory, defaultMembershipId));
        return restTemplate;
    }
    ...
    public static class InternalJwtInterceptor implements ClientHttpRequestInterceptor {
        ...
        public InternalJwtInterceptor(
                String jwtHeaderName, JWTFactory jwtFactory, String defaultMembershipId) {
        ...
        public ClientHttpResponse intercept(
            ...
            String jwt =
                    jwtFactory.generateUserToken(
                            Optional.ofNullable(defaultMembershipId)
